...
function updateEmployee($employeeInput, $customerParams){
    global $conn;
    ...
    $employeeId=mysqli_real_escape_string($conn,$customerParams['id']);
    ...
    if(empty(trim($name))){
        ...
        $query="UPDATE employees SET name='$name' , email='$email' , phone='$phone' WHERE id='$customerId' LIMIT 1";
        $result=mysqli_query($conn,$query);
